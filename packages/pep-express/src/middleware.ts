...
import { evaluate, type PolicySet, type Decision } from "@guardrails/pdp";
import { buildRequestContext } from "./context.js";
import type { GuardrailsOptions, GuardrailsRequest } from "./types.js";
import fs from "fs";
import path from "path";

/**
 * Express middleware for Guardrails policy enforcement.
 *
 * @example
 * ```typescript
 ...
 * app.use(guardrails({ policy: "./policy.yaml" }));
 * ```
 */
export function guardrails(options: GuardrailsOptions): RequestHandler {
  const policy = loadPolicy(options.policy);
  const decisionHeader = options.decisionHeader ?? "X-Guardrails-Decision";
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      (req as GuardrailsRequest).guardrails = { decision, context: ctx };
      ...
      console.error("[guardrails] Error evaluating policy:", error);
      return next();
    }
  };
}
 ...
 * Use before express.json() and guardrails().
 *
 * @example
 * ```typescript
 * app.use(rawBodyCapture());
 * app.use(express.json());
 * app.use(guardrails({ policy }));
 * ```
 */
